...
# base85-encoded (n_layers, n_heads) boolean arrays indicating the
...
def _download(url: str, root: str) -> str:
    ...
    if os.path.isfile(download_target):
        model_bytes = fread(download_target, binary=True)
...
def load_model(
    ...
    assert name in _MODELS, f"{name} not found: {available_models()}"
