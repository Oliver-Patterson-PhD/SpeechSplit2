...
def ConvTranspose2d(
    ...
    m = torch.nn.ConvTranspose2d(in_channels, out_channels, kernel_size, **kwargs)
...
class ResidualConv1dGLU(torch.nn.Module):
    ...
    def _forward(self, x, c, g, is_incremental):
        ...
        """
        residual = x
        x = torch.torch.nn.functional.dropout(x, p=self.dropout, training=self.training)
        if is_incremental:
            splitdim = -1
