import os
from typing import Any, Callable, Iterable, List, Optional, Self, Tuple
...
def has_nans(x: torch.Tensor) -> str:
...
def any_nans(xi: Iterable[torch.Tensor]) -> bool:
    for x in xi:
        if is_nan(x):
            return True
    return False
...
def speaker_normalization(
    f0: torch.Tensor, index_nonzero: torch.Tensor, mean_f0: float, std_f0: float
) -> torch.Tensor:
    f0.dtype
    std_f0 += 1e-6
...
def quantize_f0_torch(x: torch.Tensor, num_bins: int = 256) -> torch.Tensor:
    # x is logf0
    B = x.size(0)
    x = x.view(-1).clone()
    uv = x <= 0
...
def get_spmel(wav: torch.Tensor, whispercheck: bool = False) -> torch.Tensor:
...
def get_spenv(wav: torch.Tensor, cutoff: int = 3) -> torch.Tensor:
    ...
    retval = torch.nn.functional.pad(
        torchaudio.functional.resample(
            env, orig_freq=env.size(dim=-1), new_freq=DIM_FREQ
        ),
        (0, 0, 0, 1),
    )
...
def extract_f0(
    wav: torch.Tensor, fs: int, lo: int, hi: int, normalise: bool = True
) -> torch.Tensor:
    f0_rapt = torch.tensor(
        rapt(wav.cpu().numpy() * 32768, fs, HOP_LENGTH, min=lo, max=hi, otype=2),
        device=wav.device,
    )
    if not normalise:
        return f0_rapt
...
def average_f0s(f0s: List[torch.Tensor]) -> List[torch.Tensor]:
...
def get_monotonic_wav(
    x: torch.Tensor, f0: torch.Tensor, sp: torch.Tensor, ap: torch.Tensor, fs: int
) -> torch.Tensor:
    y = torch.tensor(
        pyworld.synthesize(
            f0.cpu().numpy(),
...
def warp_freq(n_fft: int, fs: int, fhi: int = 4800, alpha: float = 0.9) -> torch.Tensor:
...
def vtlp(x: torch.Tensor, fs: int, alpha: Optional[float]) -> torch.Tensor:
    if alpha is None:
        alpha = 0.2 * torch.rand(1).item() + 0.9
    vtlp_stft = torch.stft(x, n_fft=VTLP_FFT, window=vtlp_window, return_complex=True).T
    dtype = vtlp_stft.dtype
    shape_t, shape_k = vtlp_stft.shape
    f_warps = warp_freq(shape_k, fs, alpha=alpha)
    ...
    y = torch.istft(new_S.T, n_fft=VTLP_FFT, window=vtlp_window)
    if len(x) <= len(y):
        y = y[: len(x)]
    else:
