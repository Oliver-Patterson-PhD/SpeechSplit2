...
class Solver(object):
    ...
    def restore_model(self, resume_iters):
        ...
        ckpt_name = (
            f"{self.experiment}-{self.bottleneck}-{self.model_type}-{resume_iters}.ckpt"
        )
        ckpt = torch.load(
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i + 1) % self.ckpt_save_step == 0:
                ckpt_name = (
                    f"{self.experiment}-{self.bottleneck}-{self.model_type}-{i+1}.ckpt"
                )
                torch.save(
                    {
