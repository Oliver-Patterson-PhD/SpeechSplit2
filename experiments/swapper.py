...
class Swapper(Experiment):
    ...
    def save_latents(self: Self) -> None:
        data_loader = get_loader(config=self.config, singleitem=True)
        [self.save_single_latent(batch) for batch in data_loader]  # type: ignore [func-returns-value]
    ...
    def save_audios(self: Self) -> None:
        ...
        if self.use_synth_melgan:
            self.synthesizer = MelGanSynthesizer(
